...
Once this plugin is activated, any existing WP user (that was not migrated from Drupal) will still be able to login using their same password.  This is possible because phpass is one of several algorithms that Drupal supports.  However, until they change or reset their password, it will remain encrypted using [phpass](http://www.openwall.com/phpass/)—WP core's default password encryption algorithm.
